					...
					<template v-slot:default>
						<thead>
							<tr class="t-bg-[#03A593] white--text">
								<th class="white--text t-text-center">{{$t('commissionPage.date')}}</th>
								<th class="white--text t-text-center">{{$t('commissionPage.sender')}}</th>
								<th class="white--text t-text-center">{{$t('commissionPage.amount')}}</th>
								<th class="white--text t-text-center">{{$t('commissionPage.type')}}</th>
							</tr>
						</thead>
						<tbody>
							<tr
							...
							>
								...
								<td class="t-text-center" v-if="v.commission_type == 'trade'">
									IB Commission
								</td>
								<td class="t-text-center" v-else>F7</td>
							</tr>
						</tbody>
					</template>
				</v-simple-table>
			...
			>
				<v-btn text depressed @click="moreDetails = false" class="mr-5">
					<div class="d-flex align-center">
						...
						<span class="t-capitalize white--text t-text-xs mt-1">{{$t('commissionPage.back')}}</span>
					</div>
				</v-btn>
				<div>
					<div>
						<div class="d-flex flex-row align-center">
							<div class="d-flex flex-row align-center">
								<span class="white--text mx-2 t-text-xs">{{$t('commissionPage.from')}}</span>
								<v-dialog
									ref="date_from"
							...
							<div class="d-flex flex-row align-center">
								<span class="white--text mx-2 t-text-xs">{{$t('commissionPage.to')}}</span>
								<v-dialog
									ref="date_to"
				...
				<v-pagination
					...
					@input="
						(v) =>
							HISTORIES({ page: v, date_from: date_from, date_to: date_to })
					"
					total-visible="7"
					color="#03A593"
				></v-pagination>
...
export default {
	data() {
		return {
			...
			date_to: null,
			headers: [
				{ text: this.$t("commissionPage.date"), value: "created_at" },
				{ text: this.$t("commissionPage.sender"), value: "username" },
				{ text: this.$t("commissionPage.amount"), value: "amount" },
				{ text: this.$t("commissionPage.type"), value: "commission_type" },
			],
		};
	},
	methods: {
		...mapActions("franchise", ["HISTORIES"]),
		postHistories() {
			const data = {
				page: this.page,
