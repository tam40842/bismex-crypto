const svgRenderDom = document.querySelector('[svg-render]');
const svgDom = svgRenderDom.querySelector("#tree-container")
var params = {
    selector: "#tree-container",
    dataLoadUrl: tree_data,
    chartWidth: svgDom.clientWidth,
    chartHeight: svgDom.clientHeight,
    funcs: {
        showMySelf: null,
        search: null,
        closeSearchBox: null,
...
function drawOrganizationChart(params) {
    ...
    var svg = d3.select(attrs.selector)
        ...
        .call(zoomBehaviours)
        .append("g")
        .attr("transform", "translate(" + attrs.width / 2.5 + "," + 20 + ") scale(" + zoomDefault + ", " + zoomDefault + ")");;
    ...
    var tooltip = d3.select(params.selector)
        .append('div')
        ...
        function tooltipContent(item) {
            ...
            var strVar = "";
                ...
                <div class="customTooltip">
                    <div class="d-inlineflex flex-column tooltip-profile">
                        ...
                        <div class="tooltip-body">
                            ...
                            <article class="tooltip-description">
                                <div class="d-block">
                                    <b>Email:</b> ${item.email}
                                </div>
                                <div class="d-block">
                                    <b>Reference ID:</b> ${item.sponsor_code}
                                </div>
                            </article>
                        </div>
                        <div class="tooltip-footer">
                            <div class="text-center">
                                <div class="btn btn-danger btn-block btn-lg">Close</div>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    function reflectResults(results) {
        var htmlStringArray = results.map(function (result) {
            var strVar = "";
            strVar += `
                <li class="border-bottom border-muted d-flex">
                    <div class="avatar">
                        <img src="${result.imageUrl}" alt="">
                    </div>
                    <div class="flex-grow-1 d-flex flex-column justify-content-between">
                        <div class="d-block">
                            <div class="name font-lg">${result.name}</div>
                            <div class="text-muted">${result.nickname}</div>
                            <div class="text-muted font-sm">${result.area}</div>
                        </div>
                    </div>
                    <div class="d-flex align-items-center px-2">
                        <a href="javascript:void(0)" class="text-danger text-center" onclick="params.funcs.locate(${result.uniqueIdentifier})">
                            <i class="fa fa-crosshairs fa-lg"></i>
                            <div class="d-block text-xs"><b>View</b></div>
                        </a>
                    </div>
                </li>
            `
        ...
        var parentElement = get('[list-tree-filter]');
        var old = parentElement.innerHTML;
        var newElement = htmlString + old;
        parentElement.innerHTML = newElement;
        set('[list-tree-number]', "RESULT - " + htmlStringArray.length);

    }

    function clearResult() {
        set('[list-tree-content]', `<ul class="nolist d-flex flex-column flex-grow-1 tree-search-result" list-tree-filter></ul>`);
        set('[list-tree-number]', "RESULT");

    }

    function listen() {
        var input = get('[input-tree-filter]');
    ...
    function searchUsers() {
        ...
        d3.selectAll('.user-search-box')
            .transition()
            .duration(250)
            .attr('class', 'user-search-box active')
    }
    ...
    function setClassCollapsible(d) {
        let target = d3.select(this)
        if (d._children) {
            target.attr("classt", "")
        } else if (d.children) {
    ...
    function findmySelf(d) {
        ...
        show(['.customTooltip-wrapper', '.btn-action.btn-back', '.department-information'])
