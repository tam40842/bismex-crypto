								...
								<table class="table table-bordered">
									<tbody>
										...
										<tr>
											<th></th>
											<td>
												<a href="{{ route('admin.transfers.approved', ['transfers_id' => $transfers->transfer_id]) }}" class="btn btn-success text-white" id="transfer_process"><i class="fa fa-check fa-fw"></i>Duyệt lệnh</a>

												<a href="{{ route('admin.transfers.cancelled', ['transfers_id' => $transfers->transfer_id]) }}" class="btn btn-danger text-white" id="transfer_process"><i class="fa fa-times fa-fw"></i>Từ chối</a>
											</td>
										</tr>
										@endif
									</tbody>
...
@push('js')
<script>
	$(document).on('click', '#transfer_process', function(event) {
